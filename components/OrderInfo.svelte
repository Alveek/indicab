<script>
  ...
  // Отображение заказов в истории заказов
  // db.collection(orderCollection)
  //   .orderBy("timestamp", "desc")
  ...
  //     monthTotalImpsFact = 0;
  //     snapshot.docs.forEach(item => {
  //       monthTotalImpsFact += item.data().impFact;
  //     });
  //   });
  ...
  function calculateImps() {
    impPlan =
      Number((order.orderLength / (order.rapport / 1000)).toFixed()) * inksTotal;
    ...
    db.collection(orderCollection)
      ...
      .update({
        ...
        bwImpTotal,
        impStart,
        // супер "сложная" формула расчета кликов на каждую краску:
...
<div class="container">
  ...
  <button class="new-order-btn outline" on:click={
    createNewOrder} >Новый заказ</button>
  </div>
    ...
    <div class="order-info-group">
      ...
      <div class="input-field">
				...
				<!-- <input class="order-input date" type="number" name="day" value={order.day} autocomplete="off" /> -->
        <span class="date-span">{order.day < 10 ? '0' + order.day : order.day}.{order.month < 10 ? '0' + order.month : order.month}.{order.year}</span>
			</div>

      <div class="input-field">
				<label class="label" for="">Смена:</label>
				<select class="order-input" name="smena" id="order-smena" value={order.smena}>
					<option style="border-bottom: 1px solid #ccc" value="">Выберите смену</option>
			...
			<div class="input-field">
				...
				<select class="order-input" name="operator" id="order-operator" value={order.operator}>
					<option value="">Выберите оператора</option>
					<option value="Серегин">Серегин</option>
					<option value="Хрипунов">Хрипунов</option>
					<option value="Саберов">Саберов</option>
					<option value="Кудрявцев">Кудрявцев</option>
				</select>
			</div>
    </div>
			...
			<div class="input-field">
				...
				<input class="order-input" type="number" name="orderNumber" min="0" value={order.orderNumber} autocomplete="off" />
			</div>

			<div class="input-field">
				<label class="label" for="">Метраж:</label>
				<input class="order-input" type="number" name="orderLength" min="0" value={order.orderLength} autocomplete="off" />
			</div>

			<div class="input-field">
				<label class="label" for="">Хвосты ч/б:</label>
				<input class="order-input" type="number" name="bwLength" min="0" autocomplete="off" value={order.bwLength} />
			</div>

			<div class="input-field">
				<label class="label" for="">Раппорт:</label>
				<input class="order-input" type="number" name="rapport" min="0" autocomplete="off" value={order.rapport} />
			</div>

      <div class="input-field">
          <label for=""><strong>Клики:</strong></label>
      </div>

			<div class="input-field">
				<label class="label" for="">- факт:</label>
				<input class="order-input" type="number" name="impFact" min="0" autocomplete="off" value={order.impFact} />
			</div>

			<div class="input-field">
				<label class="label" for="">- запуск:</label>
				<input class="order-input no-border" type="number" name="impStart" min="0" autocomplete="off" value={order.impStart ? order.impStart : 0} disabled/>
			</div>

			<div class="input-field">
				<label class="label" for="">- тираж:</label>
        <input class="order-input no-border" type="number" name="impPlan" min="0" autocomplete="off" value={order.impPlan ? order.impPlan : 0} disabled/>
			</div>

			<div class="input-field">
				<label class="label" for="">- ч/б:</label>
        <input class="order-input no-border" type="number" name="impBW" min="0" autocomplete="off" value={bwImpTotal ? bwImpTotal : 0} disabled/>
			</div>

      <div>
        {#if !order.impFact }
          <button class="calculateImpsBtnInactive btn" disabled>Рассчитать клики</button>
        {:else}
          <button class="calculateImpsBtn btn info" on:click={calculateImps}>Рассчитать клики</button>
        {/if}
      </div>
		</div>
		...
		<div on:change={() =>alertToCalculate()} class="col-inks">
			<table>
				...
				<tr>
					...
					<td><input class="order-input" type="number" name="ink_white_qty" min="0" value={order.ink_white_qty} /></td>
					<td>{order.ink_white_imp}</td>
					<td><select class="order-input" name="ink_white_bw" value={order.ink_white_bw}>
				...
				<tr>
					<td style="border-left: 4px solid cyan; padding-left: 15px;">Cyan</td>
					<td><input class="order-input" type="number" name="ink_cyan_qty" min="0" value={order.ink_cyan_qty} /></td>
					<td>{order.ink_cyan_imp}</td>
					<td><select class="order-input" name="ink_cyan_bw" value={order.ink_cyan_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid magenta; padding-left: 15px;"> Magenta</td>
					<td><input class="order-input" type="number" name="ink_magenta_qty" min="0" value={order.ink_magenta_qty} /></td>
					<td>{order.ink_magenta_imp}</td>
					<td><select class="order-input" name="ink_magenta_bw" value={order.ink_magenta_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid yellow; padding-left: 15px;">Yellow</td>
					<td><input class="order-input" type="number" name="ink_yellow_qty" min="0" value={order.ink_yellow_qty} /></td>
					<td>{order.ink_yellow_imp}</td>
					<td><select class="order-input" name="ink_yellow_bw" value={order.ink_yellow_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid black; padding-left: 15px;">Black</td>
					<td><input class="order-input" type="number" name="ink_black_qty" min="0" value={order.ink_black_qty} /></td>
					<td>{order.ink_black_imp}</td>
					<td><select class="order-input" name="ink_black_bw" value={order.ink_black_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid orange; padding-left: 15px;">Orange</td>
					<td><input class="order-input" type="number" name="ink_orange_qty" min="0" value={order.ink_orange_qty} /></td>
					<td>{order.ink_orange_imp}</td>
					<td><select class="order-input" name="ink_orange_bw" value={order.ink_orange_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid blueviolet; padding-left: 15px;">Violet</td>
					<td><input class="order-input" type="number" name="ink_violet_qty" min="0" value={order.ink_violet_qty} /></td>
					<td>{order.ink_violet_imp}</td>
					<td><select class="order-input" name="ink_violet_bw" value={order.ink_violet_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
				...
				<tr>
					<td style="border-left: 4px solid lightgreen; padding-left: 15px;">Green</td>
					<td><input class="order-input" type="number" name="ink_green_qty" min="0" value={order.ink_green_qty} /></td>
					<td>{order.ink_green_imp}</td>
					<td><select class="order-input" name="ink_green_bw" value={order.ink_green_bw}>
						<option value="no">Нет</option>
						<option value="yes">Да</option>
					</select></td>
				</tr>
        <tr><br></tr>
				<tr>
					...
					<td><input class="order-input" type="number" name="inks_total" bind:value={inksTotal} disabled/></td>
          <td><input class="order-input imps-sum" type="text" name="imps_sum" bind:value={impsSum} disabled></td>
          <td ><button on:click={setCmyk} class="cmyk-btn btn success">CMYK</button></td>
				</tr>
			</table>
		</div>
	</div>
...
<div class="operations-input-field">
  <select class="higherSelect" id="operations" bind:value={selectedSupply}>
      ...
      <optgroup label="Прочее">
		...
		<button class="btn info" on:click={replaceSupply(selectedSupply)} style="transform: translateY(2px);">Заменить</button>
</div>
    ...
    <div class="comments">
        <h3>Комментарии</h3>
        <textarea class="order-input" name="comments" value={order.comments} cols="38" rows="8"></textarea>
      </div>
	</div> 
    ...
    <div>
      <ol class="operations-list" reversed>
        {#each replacements as item, index}
          <li data-index={index} data-name={item} class="replaced-item">{suppliesTemplate[item]} <span class="del-rep-btn" on:click={deleteReplacement}>удалить</span></li>
        {/each}
      </ol>
  </div>
  </div>
  ...
  <!---------------------- ИСТОРИЯ ЗАКАЗОВ -->
    ...
    <select class="higherSelect" name="year" bind:value={currentYear} style="width: min-content; margin-bottom: 20px;">
      <option value="2020">2020</option>
      <option value="2021">2021</option>
    </select>

    <button class="show-btn btn info" on:click={()=>{
      getOrderData();
      stats.getStats();
      supplyUsage.getSupplyUsage();
      }}>Показать</button>
      ...
      <div class:active="{currentId === order.id}" data-id={order.id} class="order-history-item" on:click={(e)=> {
        currentId = order.id;
        }}>
      <span>{order.data().day < 10 ? '0' + order.data().day : order.data().day}.{order.data().month < 10 ? '0' + order.data().month : order.data().month}.{order.data().year}</span>
